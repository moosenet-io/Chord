...
# variable (not a source literal) so this materialization script carries no
# secret-shaped `NAME=value` token for the PII gate to flag — the value itself
# is only ever read at runtime from Chord's own env, never hardcoded.
