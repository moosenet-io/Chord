...
mod tests {
    ...
    fn test_credential_openai_key_blocked() {
        let result = guard().scan("some_tool", &json!({ "api_key": "glpat-test0000example0000synthetic0" }));
        assert!(result.is_err());
    ...
    fn test_credential_github_token_blocked() {
        let result = guard().scan("some_tool", &json!({ "token": "ghp_aBcDeFgHiJkLmNoPqRsTuVwXyZ1234567890" }));
        assert!(result.is_err());
        assert_eq!(block_reason(&result), Some("credential pattern"));
    }
    ...
    fn test_credential_google_service_key_blocked() {
        let result = guard().scan("some_tool", &json!({ "credential": "gsk_abcdefghijklmnopqrstuvwx12345" }));
        assert!(result.is_err());
        assert_eq!(block_reason(&result), Some("credential pattern"));
    }
    ...
    fn test_searxng_query_still_checks_credentials() {
        ...
        let result = guard().scan("searxng_search", &json!({ "query": "glpat-test0000example0000synthetic0" }));
        assert!(result.is_err());
        assert_eq!(block_reason(&result), Some("credential pattern"));
    }
    ...
    fn test_security_event_does_not_echo_blocked_content() {
        let malicious = "glpat-test0000example0000synthetic0";
