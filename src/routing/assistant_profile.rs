...
// ─────────────────────────────────────────────────────────────────────────────
// SRV-06 chat-role PIN — REMOVED (CHRD-98)
// ─────────────────────────────────────────────────────────────────────────────
//
// `apply_chat_pin` used to live here: it took the decision above and pinned the
// chosen model in `serving::residency::VramResidencyManager`. That manager was
// never constructed anywhere in chord's history, so this half was only ever
// reachable from its own tests. It was deleted together with the manager by
// CHRD-98; `routing::resident_set::ResidentSet` (CHRD-PIN-01) is the single
// owner of VRAM residency and of the chat pin. If a chat-role pin is wanted
// again, implement it AGAINST `ResidentSet` — never by reviving a second
// residency owner.
//
// `decide_chat_role` above (the pure *which model* decision) is untouched.
