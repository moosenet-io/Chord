...
- Assistant-mode resident set (TRTR-07 — three models held resident so a tool turn does not cold-load several GB, released on a mode swap):
  ...
  - **Release always wins (lifecycle guard).** A warm pass is slow network I/O and cannot hold a lock across it, so every release bumps a generation counter; a pass that completes against a stale generation DISCARDS itself and installs nothing. Once `release` has returned, no in-flight warm can re-pin a model onto a GPU that is being handed to Harmony/MINT. Concurrent warms coalesce into a single pass rather than duplicating warm requests, and a refresh that fails preserves a slot that is still warm and valid instead of downgrading it
