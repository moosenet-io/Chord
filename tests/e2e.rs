...
//!
//! Every test uses httpmock for the remote MCP backend — no real infrastructure.
...
fn make_state(mcp_url: String) -> Arc<AppState> {
    ...
    Arc::new(AppState {
        ...
        model_registry,
        pull_coordinator,
        local_evictor: std::sync::Arc::new(chord_proxy::models::eviction::FsLocalEvictor::new(std::path::PathBuf::from("/tmp"))), disk_op_lock: chord_proxy::models::eviction::new_disk_op_lock(), disk_probe: std::sync::Arc::new(chord_proxy::models::transfer::StatvfsProbe), disk_pressure_percent: 80, model_warm_cooldown_hours: 168, model_archive_copy_timeout_secs: 1800, model_gc_min_age_secs: 300, routing_map: std::sync::Arc::new(Mutex::new(chord_proxy::serving::profile::RoutingMap::empty())), coding_profile_source: std::sync::Arc::new(Mutex::new(None)), score_source: std::sync::Arc::new(Mutex::new(None)), cold_score_source: std::sync::Arc::new(Mutex::new(None)), lumina_aliases: chord_proxy::routing::lumina_alias::LuminaAliasStore::empty(), personal_proxy: None, embeddings_config: chord_proxy::embeddings::EmbeddingsConfig::test_default(None, "http://127.0.0.1:1".to_string()),
    })
}
...
async fn test_tool_call_routes_to_mcp() {
    ...
    assert_eq!(
        json["source"], "mcp",
        "source must be 'mcp' when the remote MCP backend handled the call"
    );
}
